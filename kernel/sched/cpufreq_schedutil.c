...
static int init_nonlinear_table(struct sugov_policy *sg_policy)
{
	...
	/* Find the absolute last valid index in the frequency table array */
	...
	/*
	 * QCOM frequency table is ascending (idx 0 = min, idx max = max).
	 * Iterate backwards to correctly fill the util_freq O(1) lookup array.
	 */
	for (i = last_valid_idx; i >= 0; i--) {
		...
		/* Calculate capacity for this OPP (Linear mapping for QCOM) */
		cap = mult_frac(freq, max_cap, policy->cpuinfo.max_freq);
		if (cap == 0)
			continue;

		/* Find capacity of the next lower valid OPP */
		next_cap = 0;
		for (j = i - 1; j >= 0; j--) {
		...
		/* Map all util values from 'cap' down to 'next_cap + 1' to this frequency */
				  ...
				  unsigned long util, unsigned long max)
{
	...
	/* If no next frequency is preselected, scale utilization with DVFS headroom,
	 * clamp it to the maximum capacity, and use it as an index for direct lookup
	 * in the precomputed util-to-frequency table.
	 */
	if (next_freq) {
		freq = next_freq;
	} else {
